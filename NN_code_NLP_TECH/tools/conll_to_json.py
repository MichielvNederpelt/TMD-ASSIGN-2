...
def make_dictionary(conll_file):
    """
    create  JSON structured files from conll inputfiles and split sentences per predicate
    :param conll_file: the path to the conll file to be structured to JSON format
    :type conll_file: string (path)
    returns: nothing but creates JSON formatted files in the data directory. Names of files dependent on inputfile names.
    Also prints start and end prints for every file and JSON dumped per sentence (cna be removed if wanted)
    """
    print("going through conll file")
    ...
    while conll_as_csv != None:
        try:
            ...
            break
    print("finished going through conll file and created JSON file")
