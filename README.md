...
This approach is a simple way of extracting predicates and arguments. Unfortunately, it isn't flawless since rules are hard coded which does not provide space for exceptions. We found that some predicates had a different dependency label, which results in overlooking that predicate. Most of the time, predicates are verbs, and therefore we accounted for verbs to be roots. But some predicates were nouns, which means that these were not extracted. Another problem was labeling incorrect predicates or arguments based on the rules. for instance, we coded the dependency label 'obl' to be an argument indicator for ARG2. But not all 'obl' instances are an argument, or they are a different argument type. These flaws will also flow through the experiments. Therefore, a complete rule based system is not used to train the system on. We train the system on the provided gold labels. We do use a rule-based approach to extract the predicates and arguments that need to be predicted. 
