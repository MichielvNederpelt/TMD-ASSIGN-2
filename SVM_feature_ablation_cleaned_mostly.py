...
def create_vectorizer_and_classifier(features, labels):
    '''
    take feature-value pairs and gold labels as input and train a logistic regression classifier
...
def print_confusion_matrix(predictions, goldlabels):
    ...
    returns: nothing but prints a confusion matrix
    '''
...
def is_predicate(row, predicates_in_sentence):
    """
    find predicate in sentence and return True if there is a predicate in the sentence, False if there is none
    :param row: the row of the token to find the predicate in
    :param predicates_in_sentence: list where predicates should be appended if found in row
    returns: either True or False for predicates in sentence and the list where predicates are added
    """
    #print(row[6], row[7])
    if row[4] == '0':
...
def is_argument(predicate, row):
        """
        find argument in sentence and return True if there is a argument in the row, False if there is none
        :param row: the row of the token to find the argument in
        :param predicate: predicate to find arguments for
        returns: either True or False for argument type in token row
        """
...
def implement_features(pred_arg_structures, feature_to_index):
    """
    implement features and labels to a list of both
    :param pred_arg_structures: dictionary of predicate id and nested for predicate argument pairs
    :param feature_to_index:  dictionary of features used in classifier
    returns: a list of dicts for features and a list of labels from exact match function
    """
    features = []
    labels = []
...
def extract_features_rule_based(conllfile, selected_features):
    '''
    extract features and gold label from preprocessed conll (here: tokens only).
    :param conllfile: path to the (preprocessed) conll file
    :type conllfile: string (path)
...
def get_predicted_and_gold_labels(testfile, vectorizer, classifier, selected_features, outputfile):
    '''
    extract features and run classifier on test file returning predicted and gold labels
...
def is_exact_match(pred, arg):
    """
    find number of predicates and make match to the predicates found
    :param pred: predicate dict
    :param args: argument dict
    returns arguments when predicate is found or 'O' when no predicate is found
    """
